...
class CookingController extends Controller
{
    ...
    public function exportPdf(Request $request)
    {
        ...
        $data = Cooking::whereDate('date', $exportDate)->get();

        return Pdf::view('pdf.pemeriksaan-cooking-kettle', [
            ...
            'jenis'    => $data->isNotEmpty() ? $data->first()->jenis_produk : '-',
            'kodeProd' => $data->isNotEmpty() ? $data->first()->kode_produksi : '-',
            'waktu'    => $data->isNotEmpty() ? ($data->first()->waktu_mulai . ' - ' . $data->first()->waktu_selesai) : '-',
            'mesin'    => $data->isNotEmpty() ? $data->first()->nama_mesin : '-',
            'doc_code' => 'QR 07/03',
        ])
        ->format('a4')
        ->landscape()
        ->name('pemeriksaan-cooking-kettle-' . $exportDate . '.pdf');
    }
}
